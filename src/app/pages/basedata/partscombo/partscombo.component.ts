...
export class PartsComboComponent implements OnInit {
  ...
  onEdit(event) {
    ...
    if (event.data.partslistname) {
      ...
      this.partscombo["selectedPartsIds"] = _.join(
        _.map(this.selectedItem, "parts_id"),
        ","
      );
    }
    this.isShowAdd = true;
  ...
  onConfirm() {
    ...
    if (
      !this.partscombo.type_name ||
      !this.partscombo.item_no ||
      !this.partscombo.item_name ||
      !this.partscombo.sale_price ||
      !this.partscombo.cost_price
    ) {
      this._state.notifyDataChanged("messagebox", {
        type: "warning",
        msg: "请填写完整！",
