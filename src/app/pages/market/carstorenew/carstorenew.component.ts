...
export class CarstoreNewComponent implements OnInit, AfterViewInit {
  ...
  checkIsRetain(value) {
    ...
    return new Promise((resolve, reject) => {
      if (
        value["RetainCar"] == "是" &&
        this.carincomeobj &&
        this.carincomeobj["RetainCar"] != "是"
      ) {
        that.formService
          .getForms(`car_booking/CarIncomeId/${this.carincomeobj["Id"]}`)
          .then(
            data => {
              if (data && data.Data.length > 0) {
                if (
                  (data.Data[0]["AuditResult"] != "通过" &&
                    data.Data[0]["AuditTime"]) ||
                  data.Data[0]["IsValid"] == 0
                ) {
                  resolve();
                } else {
  ...
  onUpdate(value: { [name: string]: any }) {
    const that = this;
    if (
      !_.trim(value["SpecialComment"]) &&
      this.carincomeobj &&
      this.carincomeobj["SpecialComment"]
    ) {
      this._state.notifyDataChanged("messagebox", {
        type: "warning",
        msg: "特别备注不能为空。",
