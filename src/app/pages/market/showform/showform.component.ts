...
export class ShowFormComponent implements OnInit {
  ...
  ngOnInit() {
    if (this.formView) {
      ...
      this.formService.getFormsFieldByName(this.formname).then(
        data => {
          if (data.Data) {
            const fieldList = _.orderBy(data.Data, "OrderInd", "asc");

            //分组处理
            const grp = _.filter(fieldList, f => {
              return _.isString(f['group']) && f['group'].length > 1;
            });
            const gps = [];
            _.each(grp, g => {
            ...
            gps.forEach(g => {
              this.groupConfig.push({
                group: g,
                config: _.filter(grp, f => {
                  return f['group'] == g;
                })
              });
            });
  ...
  getFormRoles(fieldList) {
    this.formService
      .getForms("vw_form_role/ViewName/" + this.formname)
      .then(data => {
        if (data.Data) {
          let newFileList = [];
          _.each(fieldList, f => {
  ...
  getDataList() {
    ...
    if (this.formname && this.keyName && this._mainId > 0) {
      this.formService
        .getForms(`${this.formname}/${this.keyName}/${this._mainId}`)
        .then(
          data => {
